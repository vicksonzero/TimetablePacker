...
public class Input {
	...
	protected void processMeetingLine(Course currentCourse, String line) throws WrongFormatException{
		...
		if(data[1].charAt(0) == 'C'){
			try {
				currentCourse.addLecture(new Lecture(Integer.parseInt(data[0]), data[1], Day.stringToDay(data[10]), timeParts[0], timeParts[1], data[12], data[13], data[14]));
			...
			try {
				currentCourse.addTutorial(new Tutorial(Integer.parseInt(data[0]), data[1], Day.stringToDay(data[10]), timeParts[0], timeParts[1], data[12], data[13], data[14]));
			} catch (NumberFormatException e) {
				throw new WrongFormatException("CRN should be a number");
			} catch (ParseException e) {
				throw new WrongFormatException("time format should be XX:XX - XX:XX");
