...
public class TestInput {
	...
	public void testWholeInput() {
		File f = new File("src/tests/testcases/test02_3courses.txt");
		Input i = new Input(f);
	...
	public void testLecProcessMeeting() {
		...
		try {
			...
			assertEquals(1, c.getLecture().size());
			assertEquals(0, c.getTutorial().size());
		} catch (WrongFormatException e) {
			fail();
		}
	}
	...
	public void testTutProcessMeeting() {
		...
		try {
			...
			assertEquals(1, c.getTutorial().size());
			assertEquals(0, c.getLecture().size());
		} catch (WrongFormatException e) {
			fail();
		}
	}
